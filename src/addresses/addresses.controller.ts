...
import { AddressesService } from "./adresses.service";
...
export class AddressesController {
    ...
    async addAddress (@Res() res, @Body() club: AddressDto) {
        const divingClubs = await this.adresseClubService.addAddress(club);
    ...
    async updateAddress(@Param('id') id: number, @Body() updatedAddress: AddressDto, @Res() res: Response) {
        const updatedClub = await this.adresseClubService.updateAddress(updatedAddress);
        return res.status(HttpStatus.OK).json({
            message: 'Update Ok',
            item: updatedClub
        });
